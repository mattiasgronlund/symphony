...
- `DECISIONS.md` — one chapter per decision: short heading, **State** (`Proposed` / `Accepted` /
  `Applied` / `Rejected` / `Superseded`), a link to the decision's folder, and a short focused prose
  description. See the `DECISIONS.md` States legend for what each means; `Superseded` (decision 0033)
  marks a decision replaced by a later one and names its successor. `Accepted` and `Applied` answer
  different questions (decision 0165): `Accepted` says the question is closed, `Applied` says the
  normative text is reachable from `main`. The pull request that lands the text is the one that flips
  the State, so the two never drift apart.
