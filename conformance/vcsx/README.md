...
125 vectors. All are pure over their inputs: no repository, network, forge, subprocess, or
...
removed the five that exercised the from-context. Twelve more arrived with decisions 0143 and 0152,
which fixed where a substituted result lands in a front-end sequence and what such a sequence must
reach; that file is per *step* rather than per invocation, so it stays pure over its inputs like the
rest.)
