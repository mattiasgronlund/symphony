...
| Form of the `resume_token`: how it spells the three parts Section 5.5 fixes, whether it is signed, and the mechanism by which the four conditions Section 8.1 fixes are judged | 5.5, 8.1, 8.2, 8.6 | `<how the point, the root trigger and the spent flow-bound count are spelled, the trigger by its registry token; how the policy, repository, major-version and entry-point bindings are carried and compared; signed or not>` |
